...
description: Create curated Hybrid Agent Rail Skill markdown from an upstream Skill. Use when selecting an external Skill into skills/<name>.md, copying the curated template/example, enforcing required Source metadata, or validating the strict How to use format.
---
...
Create a curated Skill Markdown file under `skills/` from an upstream Skill. This Skill keeps the curated format stable by requiring an upstream Source, a strict How to use block, and validation before the file is considered complete.
...
3. Create or update `skills/<skill-name>.md`.
4. Fill only these How to use variables:
   - discovery timing summary
   - upstream URL
   - Skill name
5. Run `node .codex/skills/create-curated-skill/validate.mjs skills/<skill-name>.md`.
...
- `references/curated-skill-template.md`: copy this shape when creating a curated Skill.
