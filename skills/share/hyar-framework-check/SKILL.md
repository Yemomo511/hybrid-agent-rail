...
你需要知晓以下内容考虑做出推荐。先读取项目根目录 `.hyar/ARCH_CONTEXT.md`，已有同一前置项答案时直接复用；仍缺信息时停止推荐，只提出最少必要问题，每次对话只问 1 个问题。用户确认回答后，使用 `arch-context-collect` 记录答案，再接着询问。
...
每个待确认问题都必须包含一个“最佳实践”选项，并给出可执行默认值：

- 用户画像：最佳实践默认按“普通开发者但不懂跨端”处理，避免用完全小白路径过度解释，也不假设用户已熟悉某框架。
- 目标平台：最佳实践默认先覆盖 Android + iOS App，除非用户已明确 Web、小程序、桌面、鸿蒙或 SDK 是一等目标。
- 常用语言：最佳实践默认选择 TypeScript/React 通用生态；已有 Kotlin/Dart/Vue/原生团队事实时再覆盖。
- UI DSL：最佳实践默认选择 React 组件描述，因其通用性更高；强原生 UI 或 Compose 团队再切换。
- 原生能力：最佳实践默认按“需要少量原生能力、优先复用成熟插件”处理；不要默认极重原生或纯业务。
- 交付约束：最佳实践默认选择最小可上线范围 + 最新稳定生态，先保证 App 核心路径可交付。
...
可选回答：
1. 最佳实践：可执行默认值 + 为什么符合最小化/最新/通用原则
2. 选项 B：适用含义
3. 选项 C：适用含义
