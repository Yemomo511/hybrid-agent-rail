...
1. 项目类型：Expo CLI / Expo managed 或 CNG、React Native Community CLI、还是既有原生 App 集成 RN。
2. 版本事实：`expo`、`expo-router`、`@react-navigation/native`、`react-native` 的当前版本。
3. 平台范围：Android、iOS、Web，是否要求 universal links、browser history、SSR/SSG 或只做移动端。
4. 导航约束：是否已有复杂 React Navigation 状态、custom navigator、custom linking parser、独立 `NavigationContainer`、原生宿主导航或埋点依赖。
5. 变更目标：新建导航、加页面、迁移旧栈、修复 deep link，还是补类型和测试。

## Pre-Question Best Practice

每个前置问题都必须提供“最佳实践”选项：

- 项目类型：新 Expo/Universal App 默认 Expo Router；Community CLI 或原生宿主项目默认 React Navigation。
- 版本事实：默认读取 `package.json` 和 lockfile，不凭记忆判断当前 API。
- 平台范围：默认 Android + iOS；只有需求明确包含 Web、SEO 或分享链接时纳入 Web routing。
- 导航约束：默认保持现有导航栈；只有收益明确且迁移成本可控时迁移。
...
1. 读取项目事实：`package.json`、`app.json`/`app.config.*`、`babel.config.*`、`tsconfig.json`、`app/` 或 `src/app/`、`App.tsx`、导航入口、Android/iOS linking 配置。
...
3. 使用 Expo Router 时读取 `references/expo-router.md`，按文件路由组织 `app`/`src/app`、`_layout.tsx`、route group、dynamic route、`Link`/`useRouter`。
4. 使用 React Navigation 时读取 `references/react-navigation.md`，按版本选择 static configuration 或 dynamic API，并处理 navigator、screen、group、linking、hooks。
...
6. 验收：启动 Metro；移动端至少跑通一个主要路由跳转；涉及 deep link 时验证 scheme/universal link；涉及 Web 时验证 URL、刷新和回退；涉及迁移时保留关键页面、参数和埋点语义。

## Reference Routing

- `references/router-selection.md`：判断 Expo Router 或 React Navigation，以及不适用场景。
- `references/expo-router.md`：实现 Expo Router 文件路由、layout、动态路由、typed routes 和 SDK 56 写法。
- `references/react-navigation.md`：实现 React Navigation 7 static config、旧 dynamic API、安装和平台配置。
